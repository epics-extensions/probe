  ...
  switch(ca_field_type(ch->chId)) {
    ...
    case DBF_LONG   : 
        ...
        if (ch->adjust.updateMask & SLIDER_LABEL) {       
            wprintf(sldr->lowerLabel,"%ld",info->L.lopr);
            wprintf(sldr->upperLabel,"%ld",info->L.hopr);
        }
        if (ch->adjust.updateMask & SLIDER_VALUE) {
            wprintf(sldr->valueLabel,"%ld",ch->data.L.value);
            n = 0;
    ...
    case DBF_DOUBLE :
        ...
        if (ch->adjust.updateMask & SLIDER_LABEL) {
            wprintf(sldr->lowerLabel,ch->format.str1,info->D.lopr);
            wprintf(sldr->upperLabel,ch->format.str1,info->D.hopr);
        }
        if (ch->adjust.updateMask & SLIDER_VALUE) {
            wprintf(sldr->valueLabel,ch->format.str1,ch->data.D.value);   
...
int createSlider(atom *ch)
{
  ...
  if ((ch->adjust.upMask & ADJUST_BUTTONS_UP)) {
    printf("createSlider : buttons not destoryed yet!\n");
    return FALSE;
  }
